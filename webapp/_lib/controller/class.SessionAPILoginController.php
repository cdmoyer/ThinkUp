...
class SessionAPILoginController extends ThinkUpController {
    ...
    var $success_redir;
    /**
     * Show error JSON if no success or failure URLs specified as $_GET params.
     * Otherwise, redirect to failure or success URLs based on result with msg parameter set.
     */
    ...
    private function succeed($message) {
        ...
        if ((isset($_SESSION["MODE"]) && $_SESSION["MODE"] == "TESTS") || getenv("MODE")=="TESTS") {
            $header_key = "TestLocation";
        } else {
            $header_key = "Location";
        }
        header($header_key.': '.$this->success_redir.$param_chainlink."msg=".urlencode($message));
    }
    ...
    private function fail($message) {
        ...
        header($header_key.': '.$this->failure_redir.$param_chainlink."msg=".urlencode($message));
    }
}
